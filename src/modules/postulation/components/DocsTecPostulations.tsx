...
export const DocsTecPostulations: FC<DocsTecPostulations> = ({ dataDoc, documentPos }) => {
    ...
    return (
        ...
        >
            {challenge?.map((item2: any, i: any) => {
                return (
                    item2.retdoc_tipo_formulario === 2 && (
                        <div key={i}>
                            <ComponetCard
...
const DocumentPostulation = ({ item2 }: any) => {
    const [visibleCheckPostulations, setVisibleCheckPostulations] = useState<boolean>(false);
    const dispatch = useDispatch<any>();
    const challenge: any = useSelector((store: any) => store.postulation.challenge.value);
    const onUploadFileChange = async (e: any) => {
        ...
        try {
            ...
            setVisibleCheckPostulations(true);
        } catch (error) {
            console.log(error);
        }
    };
    return (
        <div>
            {visibleCheckPostulations && <ContainerCheckDocument />}
            <div>
                <span
                    style={{
                        width: '100%',
